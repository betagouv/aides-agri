...
class BaseLoader:
    ...
    def _upsert(self, code: str, row: dict) -> bool:
        _, is_new = Territoire.objects.update_or_create(
    ...
    def load(self):
        ...
        delete_qs = Territoire.objects.filter(type=self.zone_geographique_type).exclude(
            code__in=codes
        )
        if delete_qs.exists():
            print(
...
class RegionsLoader(BaseLoader):
    entity_name = "Régions"
    zone_geographique_type = Territoire.Type.REGION
    geoapi_entity = "regions"


class DepartementsLoader(BaseLoader):
    entity_name = "Départements"
    zone_geographique_type = Territoire.Type.DEPARTEMENT
    geoapi_entity = "departements"

    def __init__(self):
        self.regions_ids_by_code = {
            zone.code: zone.pk
            for zone in Territoire.objects.filter(type=Territoire.Type.REGION)
        }
...
class CollectivitesOutreMerLoader(BaseLoader):
    ...
    zone_geographique_type = Territoire.Type.COM
...
class EpcisLoader(BaseLoader):
    entity_name = "EPCIs"
    zone_geographique_type = Territoire.Type.EPCI
    geoapi_entity = "epcis"


class CommunesLoader(BaseLoader):
    entity_name = "Communes"
    zone_geographique_type = Territoire.Type.COMMUNE
    geoapi_entity = "communes"

    def __init__(self):
        self.departements_ids_by_code = {
            zone.code: zone.pk
            for zone in Territoire.objects.filter(type=Territoire.Type.DEPARTEMENT)
        }
        self.coms_ids_by_code = {
            zone.code: zone.pk
            for zone in Territoire.objects.filter(type=Territoire.Type.COM)
        }
        self.epcis_ids_by_code = {
            zone.code: zone.pk
            for zone in Territoire.objects.filter(type=Territoire.Type.EPCI)
        }
    ...
    def _upsert(self, code: str, row: dict) -> bool:
        ...
        _, is_new = Territoire.objects.get_or_create(
            type=Territoire.Type.COMMUNE,
            code=row["code"],
            nom=row["nom"],
            parent_id=parent_id,
            intercommunalite_id=self.epcis_ids_by_code[row["codeEpci"]]
            if "codeEpci" in row
            else None,
        )
        return is_new

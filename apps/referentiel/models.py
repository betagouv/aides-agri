...
class DemarcheAgricoleQuerySet(models.QuerySet):
    def to_do(self):
        return self.filter(
            status__in=(DemarcheAgricole.Status.NEW, DemarcheAgricole.Status.UPDATED)
        )

    def done(self):
        return self.filter(status=DemarcheAgricole.Status.DONE)
...
class DemarcheAgricole(models.Model):
    ...
    objects = DemarcheAgricoleQuerySet.as_manager()

...
from .models import Aide, ZoneGeographique, Type, Filiere
...
class ParentAideDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if filtered_beneficiaires:
            q_children &= Q(
                beneficiaires__contains=[
                    Aide.Beneficiaire[b].value for b in filtered_beneficiaires
                ]
            )
        ...
        context_data.update(
            {
                ...
                "filter_beneficiaires": [
                    (
                        Aide.Beneficiaire(beneficiaire).name,
                        Aide.Beneficiaire(beneficiaire).value,
                        Aide.Beneficiaire(beneficiaire).value,
                    )
                    for beneficiaire in set(
                        itertools.chain.from_iterable(
                            [
                                items
                                for items in children.values_list(
                                    "beneficiaires", flat=True
                                )
                                if items
                            ]
                        )

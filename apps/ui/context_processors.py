...
def ui_tools_tokens(request):
    ...
    if settings.UI_DEMO_SITE_MODAL_COOKIE_NAME:
        context_data.update(
            {
                "UI_SHOW_DEMO_SITE_MODAL": True,
                "UI_OPEN_DEMO_SITE_MODAL": settings.UI_DEMO_SITE_MODAL_COOKIE_NAME
                not in request.COOKIES,
            }
        )
    return context_data

...
def _find_cibles(raw_cibles: str) -> Demarche.Cible | int:
...
def _find_categories_entreprise(
    ...
    raw_categories_entreprise = raw_categories_entreprise.split("|")
    categories_entreprise = 0
    missing_raw_categories_entreprises = set()
    for raw_categorie_entreprise in raw_categories_entreprise:
        try:
            categories_entreprise |= Demarche.Cible[raw_categorie_entreprise.upper()]
        except ValueError:
            missing_raw_categories_entreprises.add(raw_categorie_entreprise)
    if missing_raw_categories_entreprises:
        raise ValueError(
            f"Some cibles are missing: {missing_raw_categories_entreprises}"
        )
    return categories_entreprise


def _find_types_aides(raw_types_aides: str) -> Demarche.Type | int:
...
def _create_porteurs(
    raw_porteurs: list[dict[str, Any]], demarche: Demarche
) -> list[Porteur]:
    to_create = []
...
@transaction.atomic
def create_demarche(raw_demarche: RawDemarcheSchemaDispositifAide) -> Demarche:
    demarche = Demarche()
    ...
    demarche.eligibilite_age_minimal = raw_demarche.eligibilite_age_minimal
    demarche.eligibilite_effectifs_minimaux = (
        raw_demarche.eligibilite_effectifs_minimaux
    )
    demarche.eligibilite_effectifs_maximaux = (
        raw_demarche.eligibilite_effectifs_maximaux
    )
    demarche.types_aides = _find_types_aides(raw_demarche.types_aides)
    demarche.cibles = _find_cibles(raw_demarche.cibles)
    demarche.eligibilite_categorie_entreprise = _find_categories_entreprise(
        raw_demarche.eligibilite_categorie_entreprise
    )
    demarche.save()
    ...
    raw_demarche.status = RawDemarcheSchemaDispositifAide.Status.DONE

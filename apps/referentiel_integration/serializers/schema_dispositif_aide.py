...
class RawDemarcheSchemaDispositifAideSerializer(serializers.ModelSerializer):
        ...
        for json_obj in value:
            if not isinstance(json_obj, dict):
                ...
                if set(json_obj.keys()) != dict_keys:
                    raise serializers.ValidationError(
                        f"Les éléments de la liste JSON de ce champ doivent avoir pour attributs {sorted(dict_keys)}"
                    )
        return value
    ...
    def validate_base_juridique(self, value):

...
class GristLoader:
    ...
    def _load_field(self, column_name: str, field: Field) -> bool:
        if getattr(self.current_obj, field.name) != self.current_row[column_name]:
            setattr(self.current_obj, field.name, self.current_row[column_name])
            return True
        return False

    def _load_fk(self, column_name: str, field: Field) -> bool:
        fk = getattr(self.current_obj, field.name)
        if self.current_row[column_name] and (
            fk is None or fk.external_id != self.current_row[column_name]
        ):
